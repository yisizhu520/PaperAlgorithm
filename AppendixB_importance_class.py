...
def test_accuracy_by_importance_feature(antibodies, test_data, parameters):
    error_count = 0
    correct_count = 0
    for x in test_data:
        yhat = predict_by_importance_feature(antibodies, x, parameters)
        # yhat = predictByVote(antibodies, x, parameters)
        if x[0] != yhat:
            error_count = error_count + 1
            # print ("predicted: ", yhat, " actual: ", x[0], "\t\t#")
...
def predict_by_importance_feature(antibodies, x, parameters):
    distances = []
    for a in antibodies:
        x_feature = get_importance_feature_data(x, a[0])
        d = distance(x_feature[1], a[1], parameters)
        if d <= a[2]:
            return a[0]
        else:
            distances.append([a[0], d])
...
def generate_population_by_importance_feature(training_set, classes, size, parameters):
    antibodies = []
    num_of_antibodies = int(float(size) / float(len(classes)))
    ...
    for c in classes:
        ...
        class_data = get_importance_feature_data_set(class_data, c)
        non_class_data = get_importance_feature_data_set(non_class_data, c)
        tree = KDTree.construct_from_data(non_class_data)
...
def get_importance_feature_data(origin_data, cls):
    features = class_features_dict[cls]
    if len(features) == 0:
        return origin_data
    result = []
    result.append(origin_data[0])
    data = []
    for i in range(0, len(features)):
        data.append(origin_data[1][feature_index_dict[features[i].strip('\n')]])
    result.append(data)
    return result


def get_importance_feature_data_set(origin_data_set, cls):
    df = pd.read_csv('class_importance/' + cls + '.csv')
    features = df['feature'].map(str.strip).values
    if len(features) == 0:
        return origin_data_set
    result_set = []
    for origin_data in origin_data_set:
        result = []
        result.append(origin_data[0])
        data = []
        for i in range(0, len(features)):
...
def get_class_features_dict(classes):
    class_features_dict = {}
    for cls in classes:
        df = pd.read_csv('class_importance/' + cls + '.csv')
        features = df['feature'].values
        class_features_dict[cls] = features
    return class_features_dict

# new structure of antibody: [ class, [x1, x2, x3,... ], radius]
original_data = getdata("data/kddcup_4k.csv")
classes = get_class_labels(original_data)
feature_index_dict = get_feature_index_dict()
class_features_dict = get_class_features_dict(classes)
...
for pop_size in range(300, 750, 50):
    ...
    for st in range(iteration_count):
        ...
        # test with GA
        ga_antibodies = generate_population_by_importance_feature(training_set, classes, pop_size, parameters)
        start = time.time()
        accuracy = test_accuracy_by_importance_feature(ga_antibodies, test_set, parameters)
        end = time.time()
